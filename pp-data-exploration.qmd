...
pp <- read_excel(here("data/processed_products/westcoast_pps.xlsx"))
...
The products data has significantly fewer processors included than the employment and address data. The employment data is missing about 400 processors included the address data. The employment data dates 1970-2022 while the products data dates 2011-2022. This could account the large discrepancy in number of processors. The products data also seems to be filtered to WC while the employment data may include all states.
...
emp_states <- left_join(pp_employment, pp_address, by = "pp_idnum")
...
wc_emp <- emp_states %>% 
  filter(plant_state_abrv %in% c("OR", "WA", "CA") | (plant_city == "AT SEA PROCESSOR" & (is.na(plant_state_abrv))))
...
avg_emp_city <- processor_zip_coords %>% 
  group_by(longitude, latitude, plant_city, plant_zip) %>% 
...
revenue <- ggplot(state_revenue, aes(x = year, y = wholesale, group = state, color = state)) +
  geom_line(linewidth = 1) +
  scale_color_viridis_d() +
...
revenue_date <- ggplot(state_revenue, aes(x = date, y = wholesale, group = state, color = state)) +
  geom_line(linewidth = 1) +
  scale_color_viridis_d() +
    scale_x_date(date_breaks = "1 year", 
               date_labels = "%Y", 
               limits = c(as.Date("2011-01-01"), as.Date("2022-12-31")), 
               expand = c(0,0)) +
  ggtitle("Yearly Wholesale Sum by State") +
  xlab("Year") +
  ylab("Wholesale ($10M)") +
...
state_avg_emp_2011 <- ggplot(monthly_avg, aes(x = date, y = avg_emp, group = plant_state_abrv, color = plant_state_abrv)) +
  geom_line(linewidth = 1) +
  scale_color_viridis_d() +
  scale_x_date(date_breaks = "1 year", 
               date_labels = "%Y", 
...
state_avg_emp_2011 / revenue_date
```
...
state_avg_emp_2011 / products_date
```
