...
ggsave(here("plots/presentation/12-2/sardine_timeseries.png"), sardine_ts)
sardine_ts
```
...
ggsave(here("plots/presentation/12-2/salmon_timeseries.png"), salmon_ts)
salmon_ts
```
...
ggsave(here("plots/presentation/12-2/hake_timeseries.png"), hake_ts)
hake_ts
```
...
tuna_ts <- ggplot() +
  ...
  geom_line(data = ft_tuna_lb, aes(x = pacfin_year, y = poundage, color = "Fish Tickets")) +
  scale_color_manual(values = c("Processed Products" = "cornflowerblue", "Fish Tickets" = "firebrick")) +
  scale_y_continuous(labels = label_number(scale_cut = cut_short_scale())) +
...
ggsave(here("plots/presentation/12-2/tuna_timeseries.png"), tuna_ts)
tuna_ts
```
...
```{r}
pp_squid <- pp %>% 
  filter(product_general == "SQUID")

pp_squid_lb <- pp_squid %>% 
  group_by(year) %>% 
  summarise(poundage = sum(pounds))
...
ft_squid_lb <- ft_squid %>% 
  group_by(pacfin_year) %>% 
  summarise(poundage = sum(landed_weight_lbs))


squid_ts <- ggplot() +
  geom_line(data = pp_squid_lb, aes (x = year, y = poundage, color = "Processed Products")) +
  geom_line(data = ft_squid_lb, aes(x = pacfin_year, y = poundage, color = "Fish Tickets")) +
  scale_color_manual(values = c("Processed Products" = "cornflowerblue", "Fish Tickets" = "firebrick")) +
  scale_y_continuous(labels = label_number(scale_cut = cut_short_scale())) +
  labs(title = "Squid Landed and Processed") +
  xlab("Year") +
  ylab("Total Weight (lbs)") +
  theme_minimal() +
  theme(legend.title = element_blank())

ggsave(here("plots/presentation/12-2/squid_timeseries.png"), squid_ts)
squid_ts
```
...
crab_ts <- ggplot() +
  geom_line(data = pp_crab_lb, aes (x = year, y = poundage, color = "Processed Products")) +
  geom_line(data = ft_crab_lb, aes(x = pacfin_year, y = poundage, color = "Fish Tickets")) +
  scale_color_manual(values = c("Processed Products" = "cornflowerblue", "Fish Tickets" = "firebrick")) +
  scale_y_continuous(labels = label_number(scale_cut = cut_short_scale())) +
  labs(title = "Crab Landed and Processed") +
  xlab("Year") +
  ylab("Total Weight (lbs)") +
  theme_minimal() +
  theme(legend.title = element_blank())

ggsave(here("plots/presentation/12-2/crab_timeseries.png"), crab_ts)
crab_ts
```
...
```{r}
pp_gf <- pp %>% 
  filter(specific_species == "GROUNDFISH")
...
gf_ts <- ggplot() +
  geom_line(data = pp_gf_lb, aes (x = year, y = poundage, color = "Processed Products")) +
  geom_line(data = ft_gf_lb, aes(x = pacfin_year, y = poundage, color = "Fish Tickets")) +
  scale_color_manual(values = c("Processed Products" = "cornflowerblue", "Fish Tickets" = "firebrick")) +
  scale_y_continuous(labels = label_number(scale_cut = cut_short_scale())) +
  labs(title = "Groundfish Landed and Processed") +
  xlab("Year") +
  ylab("Total Weight (lbs)") +
  theme_minimal() +
  theme(legend.title = element_blank())

ggsave(here("plots/presentation/12-2/gf_timeseries.png"), gf_ts)
gf_ts
```
...
albacore <- plot_gf_decades_faceted(
  "ALBACORE",
  decades_list = list(
    c(1981,1989),
    c(1990,1999),
...
albacore
ggsave(here("plots/1_13_update/albacore_land_proc.png"), albacore, width = 14, height = 12, units = "in")
```
...
```{r}
# Ft_species_coords has fish ticket coordinates
# pp has coords
# both have product_general



ft_lat <- ft_species_coords %>% 
  group_by(pacfin_year, product_general) %>% 
  summarise(weight_lat = weighted.mean(lat, w = landed_weight_lbs, na.rm = TRUE)) %>% 
  rename(year = pacfin_year) %>% 
  mutate(dataset = "fish tickets")

pp_lat <- pp %>% 
  group_by(year, product_general) %>% 
  summarise(weight_lat = weighted.mean(lat, w = pounds, na.rm = TRUE)) %>% 
  mutate(dataset = "processed products")


latitudes <- rbind(ft_lat, pp_lat)

salmon_lat <- ft_lat %>% 
  filter(product_general == "SALMON") %>% 
  select(year, lat_ft = weight_lat) %>% 
  left_join(
    pp_lat %>% 
      filter(product_general == "SALMON") %>% 
      select(year, lat_pp = weight_lat),
    by = "year"
  )


ggplot(salmon_lat) +
  geom_segment(aes(x = year, xend = year, y = lat_ft, yend = lat_pp),
               color = "#aeb6bf",
               size = 2,
               alpha = 0.5) +
  geom_point(aes(x = year, y = lat_ft),color = "firebrick", size = 4, show.legend = TRUE) +
  geom_point(aes(x = year, y = lat_pp), color = "cornflowerblue", size = 4, show.legend = TRUE) +
  theme_minimal()
```

## Function for dumbbell plot
```{r}
plot_latitudes <- function(species_name) {
  species_lat <- ft_lat %>% 
  filter(product_general == species_name) %>% 
  select(year, lat_ft = weight_lat) %>% 
  left_join(
...
ggplot(species_lat) +
  ...
  geom_point(aes(x = year, y = lat_pp), color = "cornflowerblue", size = 4, show.legend = TRUE) +
  ylim(32, 48) +
  labs(title = paste0(species_name, " Processing & Landings Latitude"),
       x = "Year",
       y = "Latitude") +
       theme_minimal()
...
```{r}
plot_latitudes("SALMON")

plot_latitudes("TUNA")

plot_latitudes("SARDINE")

plot_latitudes("HAKE")

plot_latitudes("SQUID")

plot_latitudes("CRAB")
```


### Groundfish Latitudes
```{r}
ft_lat_gf <- ft_species_coords %>% 
  group_by(pacfin_year, specific_species) %>% 
  summarise(weight_lat = weighted.mean(lat, w = landed_weight_lbs, na.rm = TRUE)) %>% 
  rename(year = pacfin_year) %>% 
  mutate(dataset = "fish tickets")

pp_lat_gf <- pp %>% 
  group_by(year, specific_species) %>% 
  summarise(weight_lat = weighted.mean(lat, w = pounds, na.rm = TRUE)) %>% 
  mutate(dataset = "processed products")


plot_latitudes_gf <- function(species_name) {
  species_lat <- ft_lat_gf %>% 
  filter(specific_species == species_name) %>% 
  select(year, lat_ft = weight_lat) %>% 
  left_join(
    pp_lat_gf %>% 
...
plot_latitudes_gf("ALBACORE")
```



THINGS TO NOTE: pp uses weight after processing. pp lat and lon off zip code but ft off iopac port group


```{r}
#Albacore weighted centroids

# Landings (fish tickets)
ft_centroids <- ft_species_coords %>% 
  filter(specific_species == "ALBACORE",
         pacfin_year %in% c(1985, 2015)) %>% 
  group_by(pacfin_year) %>% 
  summarise(
    lon = weighted.mean(lon, w = landed_weight_lbs, na.rm = TRUE),
    lat = weighted.mean(lat, w = landed_weight_lbs, na.rm = TRUE),
    .groups = "drop"
  ) %>% 
  mutate(
    dataset = "Landings",
...
pp_centroids <- pp %>% 
  ...
  group_by(year) %>% 
  summarise(
    lon = weighted.mean(lon, w = pounds, na.rm = TRUE),
    lat = weighted.mean(lat, w = pounds, na.rm = TRUE),
    .groups = "drop"
  ) %>% 
  mutate(dataset = "Processing")
albacore_centroids <- bind_rows(ft_centroids, pp_centroids)
albacore_centroids_sf <- st_as_sf(
  albacore_centroids,
  coords = c("lon", "lat"),
  crs = 4326
)
...
ggplot() +
  geom_sf(data = wc_states, fill = "gray95", color = "gray60", linewidth = 0.3) +
  
  geom_sf(
    data = albacore_centroids_sf,
    aes(color = dataset, shape = factor(year)),
    size = 4
  ) +
  ...
  labs(
    title = "Albacore Processing vs Landings Centroids",
    subtitle = "Weighted by pounds, 1985 and 2015",
    color = NULL,
    shape = "Year"
  ) +
